...
[![pav:version](http://practicalprovenance.files.wordpress.com/2013/12/pav-versions-simple-new-page.png)](http://practicalprovenance.files.wordpress.com/2013/12/pav-versions-simple-new-page.png)
...
[![PAV versions](http://practicalprovenance.files.wordpress.com/2013/12/pav-versions-new-page-2.png)](http://practicalprovenance.files.wordpress.com/2013/12/pav-versions-new-page-2.png)
...
[![PAV dataset](http://practicalprovenance.files.wordpress.com/2014/01/pav-dataset-lineage-new-page-2.png)](http://practicalprovenance.files.wordpress.com/2014/01/pav-dataset-lineage-new-page-2.png)

Example of using PAV to version datasets, showing the provenance of each individual version. [doi:10.6084/m9.figshare.894329](http://dx.doi.org/10.6084/m9.figshare.894329)
...
Our derivation subproperty [pav:derivedFrom](http://pav-ontology.googlecode.com/svn/trunk/pav.html#http://purl.org/pav/derivedFrom "pav:derivedFrom") is again intentionally more specific, requiring a significant change in content, and thus can be used to clarify the level of change.

We made a [mapping to PROV-O](https://jbiomedsem.biomedcentral.com/articles/10.1186/2041-1480-4-37#Tab4) which explains the rationale for each PAV subproperty.
...
[![](http://practicalprovenance.files.wordpress.com/2014/01/pav-and-prov-qualified-revisions-qualifiedrevision.png)](http://practicalprovenance.files.wordpress.com/2014/01/pav-and-prov-qualified-revisions-qualifiedrevision.png)
...
This kind of qualification pattern can be also be used for other PAV properties that have PROV superproperties, such as [prov:qualifiedDerivation](http://www.w3.org/TR/prov-o/#qualifiedDerivation) on [pav:importedFrom](//purl.org/pav/importedFrom "pav:importedFrom"), or [prov:qualifiedAttribution](http://www.w3.org/TR/prov-o/#qualifiedAttribution) on [pav:authoredBy](//purl.org/pav/authoredBy "pav:authoredBy"), however in many cases it might be better to expand the change by relating entities to [PROV activities](http://www.w3.org/TR/prov-primer/#activities).

### DC Terms

The [Dublin Core Terms](http://dublincore.org/documents/dcmi-terms/) is a well-established and popular vocabulary to provide bibliographic records, particularly for document-like resources. As its focus is on human-readable bibliographies rather than provenance, there is not necessarily a 'backwards in time' lineage when using DC Terms relations. These DC Terms properties can be used for describing versions of resources:
...
![PAV versions - hasEarlierVersion](https://practicalprovenance.files.wordpress.com/2016/05/pav-versions-hasearlierversion.png)
...
```
![PAV versions - hasVersion](https://practicalprovenance.files.wordpress.com/2016/05/pav-versions-hasversion.png)
```
...
![PAV-hasCurrentVersion](https://practicalprovenance.files.wordpress.com/2016/05/pav-hascurrentversion.png)
...
It can be useful to have third-party "versions" (e.g. forks in software development) - where you would not find the official _pav:hasVersion_ statement from the . In this case you should add a _prov:specializationOf_ backlink and _pav:derivedFrom_ statement to which version you forked.

### Hierarchies all the way down

There is nothing preventing you from also using _pav:hasVersion_ to define deeper hierarchies, e.g. for software using semantic versioning:

https://gist.github.com/stain/282be617dfcd6b3129b685f41fdefeaa

But this raises some challenges with _pav:previousVersion_, _pav:hasCurrentVersion_ and _pav:version._

I would suggest this pattern for representing semantic versioning hierarchically:

https://gist.github.com/stain/66241c56d83c1c5803c4085e520bbf4a

.. as _pav:hasCurrentVersion_ should point to the permalink snapshot in a functional way, it would be confusing to also include its "current version" as "2" and "2.1". So I suggest to let it always point to the "deepest" version. _pav:version_ of the intermediaries should show the latest version of their _pav:hasCurrentVersion_ - not a generic "2" or "2.1". (You can use _rdfs:label_ to say "2.1").

For the 'abandoned' versions, _pav:hasCurrentVersion_ and _pav:version_ would be the latest one within their level:

https://gist.github.com/stain/c57516cedcd45fd05bbfcf9fb7f4d7cb
